...
exports.default = function (_ref) {
  ...
  var buildDepRequires = function buildDepRequires(deps) {
    var depRequires = [];
    ...
    visitor: {
      CallExpression: {
        enter: function enter(path, _ref2) {
          ...
          if (this.outerModuleDefinition) return;
          ...
          if (t.isIdentifier(callee, { name: 'define' }) && !path.scope.hasBinding('define') && args.length >= 1 && args.length <= 3) {
            ...
            if (opts.filterMode) {
                  ...
                  if (requireParam) {
                    var filterAMDamdDeps = {
                      ...
                      CallExpression: function CallExpression(path) {
                        ...
                        if (path.scope.bindings[requireParam.name] && path.scope.bindings[requireParam.name].identifier === requireParam && t.isIdentifier(callee, { name: requireParam.name })) that.amdDeps.push(args[0].value);
                      }
                    };
              ...
              (function () {
                ...
                if (args.length === 1) {
                  ...
                  if (t.isFunctionExpression(factoryArg)) {
                    factoryArg.params.forEach(function (factoryParam) {
                      if (t.isIdentifier(factoryParam) && factoryParam.name === 'module') isModuleInDepsOrInFactoryParam = true;
                    });
                  }
                } else if (args.length === 2) {
                  // first param is either module name or dependency array
                  if (t.isStringLiteral(args[0])) moduleName = args[0];else if (t.isArrayExpression(args[0])) deps = args[0];
                ...
                var factory = buildFactory({
                  ...
                  DEP_REQUIRES: buildDepRequires(deps.map(function (dep) {
                    return dep.value;
                  }).filter(function (dep) {
                    return factoryRequired.indexOf(dep) === -1;
                  })),
        ...
        exit: function exit(path) {
          ...
          if (this.outerModuleDefinition === path) this.outerModuleDefinition = null;
        }
      },
      MemberExpression: function MemberExpression(path, _ref3) {
        ...
        if (!opts.filterMode) {
          ...
          if (t.isIdentifier(path.node.object, { name: 'define' }) && t.isIdentifier(path.node.property, { name: 'amd' }) && !path.scope.hasBinding('define') && path.parentPath && t.isLogicalExpression(path.parentPath)) path.replaceWith(t.booleanLiteral(true));
        }
      },
